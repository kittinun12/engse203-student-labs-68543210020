...
export function readStoredRequests() {
  ...
  try {
    ...
    if (!envelope || typeof envelope !== 'object') {
      return { status: 'invalid', reason: 'โครงสร้างข้อมูลไม่ถูกต้อง' };
    }

    if (envelope.schemaVersion !== SCHEMA_VERSION) {
      return { status: 'invalid', reason: 'เวอร์ชันของ schema ไม่ตรงกัน' };
    }

    if (!validateRequests(envelope.requests)) {
      return { status: 'invalid', reason: 'ข้อมูลคำร้องไม่ถูกต้องตามข้อกำหนด' };
    }
